...
class CNN1D(nn.Module):
    def __init__(
        ...
        self._conv1 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=128, kernel_size=4, padding=2),
            nn.BatchNorm1d(128),
            nn.ReLU(),
        ...
        self._conv5 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=100, kernel_size=1, padding=1),
            nn.BatchNorm1d(100),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2)
        )
    ...
    def forward(self, x):
        ...
        x = x.squeeze(1) # batch x freq x time
        x = self._conv1(x) 
        x = self._conv2(x) 
        x = self._conv3(x) 
        x = self._conv4(x)
        x = self._conv5(x) 
        embedding = self._pooling(x)
